<?php get_header();  the_post();  ?>
  ...
  <div style="background: url(<?php echo $img3['url']; ?>) no-repeat center center; background-size:cover;" class="box size-2">
  	<!--Image 3-->
  </div>
    <div style="background: url(<?php echo $img2['url']; ?>) no-repeat center 60%; background-size:cover;" class="box size-1">
  	<!--Image 2-->
  </div>
		 <?php
	  $hl = get_field('headline_or_logo'); ?>
  <div class="box headline<?php if($hl == 'Logo') { echo ' logo-only'; } ?>">
 <?php
	   if($hl == 'Headline') { ?>
	  <h2><?php the_field('headline'); ?></h2>
	  <p><?php the_field('banner_content'); ?></p>
	     <?php } elseif($hl == 'Logo') { ?>
	     	<img src="<?php $mylogo = get_field('logo'); echo $mylogo['url']; ?>"/>
	     <?php } ?>
  </div>
...
<?php }?>
  ...
  <?php the_field('our_history'); ?>
    ...
    <section class="gallery-sliders">
    	<div class="container">
    	<h2>Gallery</h2>
    		<div class="gallery-tabs">
    			<ul>
    				<li><a href="#property" class="r-tabs-state-active">Property</a></li>
    			...
    			<div class="slider-tabs">
    				<div id="property">
    					...
    					<?php
	    					if(have_rows('gallery')) : while(have_rows('gallery')) : the_row(); if(get_sub_field('category') == 'property') {
	    					$imgSrc = get_sub_field('image');
	    					$imageUrl = $imgSrc['url'];
	    					$image = $imgSrc['sizes']['new-gal'];
    					?>
    						<div><a href="<?php echo $imageUrl; ?>"><img src="<?php echo $image; ?>"/></a></div>
    					<?php } endwhile; endif; ?>
    					</div>
    				</div>
    				<div id="event">
    					<div class="three-slider">
    						<?php
	    					if(have_rows('gallery')) : while(have_rows('gallery')) : the_row(); if(get_sub_field('category') == 'events') {
	    					$imgSrc = get_sub_field('image');
	    					$imageUrl = $imgSrc['url'];
	    					$image = $imgSrc['sizes']['new-gal'];
    					?>
    ...
    <div id="pricing"></div>
     <section id="<?php print (str_replace(' ', '-', strtolower(get_sub_field('tab_title')))); ?>" class="pricing <?php if(get_sub_field('color') == 'green') { echo 'white-text'; } ?>" <?php if($bgimg = get_sub_field('background_image')) { echo 'style="background: url('.$bgimg.') no-repeat center center; background-size: cover;"'; } ?>>
  <div class="container">
...
<?php endif; ?>
    ...
    <?php } elseif(get_row_layout() == 'row_content') { ?>



   	<section class="row-content" <?php if($bgimg = get_sub_field('background_image')) { echo 'style="background: url('.$bgimg.') no-repeat center center; background-size: cover; "'; } ?>>
   		<div class="container">
   			<h2><?php the_sub_field('row_header'); ?></h2>
   			<?php the_sub_field('row_content_editor'); ?>
   		</div>
   	</section>
    ...
    <section id="testimonial" class="testimonial-section">
    	<div class="container">
            <h2>Testimonails</h2>
    		<ul id="testimonial-section-slider">
                <?php while(have_posts()) : the_post(); ?>
                <li>
				    <blockquote><?php the_field('quote'); ?></blockquote>
				    <?php if($cite = get_field('citation')) { echo '<cite>'.$cite.'</cite>'; } ?>
				</li>
            <?php endwhile; ?>
    		</ul>
    	</div>
    ...
    <div class="container form-wrapper">
        <form id="form1" class="wufoo topLabel page" accept-charset="UTF-8" autocomplete="off" enctype="multipart/form-data" method="post" novalidate="" action="https://lockefallsfarm.wufoo.com/forms/zjnffcz18vzq3p/#public">
            <ul>
                ...
                <li id="foli8" class="date notranslate"><label class="desc" id="title8" for="Field8">Event Date</label> <span class="month"><input id="Field8-1" placeholder="MM" name="Field8-1" type="text" class="field text" value="" size="2" maxlength="2" tabindex="4"> <label for="Field8-1">MM</label></span> <span><input id="Field8-2" placeholder="DD" name="Field8-2" type="text" class="field text" value="" size="2" maxlength="2" tabindex="5"> <label for="Field8-2">DD</label></span> <span><input id="Field8" placeholder="YYYY" name="Field8" type="text" class="field text" value="" size="4" maxlength="4" tabindex="6"> <label for="Field8">YYYY</label></span> <img id="pick8" class="datepicker" src="<?php echo get_stylesheet_directory_uri(); ?>/img/wufoo/calendar.png" alt="Pick a date." data-date-format="yyyy-mm-dd" data-date="2015-12-06"></li>
        ...
        <div class="contact-info">
            ...
            <a href="mailto:info@lockefallsfarm.com">info@lockefallsfarm.com</a></p>
        </div>
    </div>
